...
@section('contain')
    ...
    <table class="table table-striped col-lg-12  mx-auto">
        ...
        @foreach($post as $post_show)
            <tr>
                <td>{{$post_show->id}}</td>
                <td>{{$post_show->name}}</td>
                <td>{{$post_show->title}}</td>
                <td>{{$post_show->details}}</td>
                <td><img src="{{URL::to($post_show->image)}}" style="height: 40px;width: 70px"></td>
                <td>{{$post_show->created_at}}</td>
                <td>
                    <a href="{{url('/edit_post/'.$post_show->id)}}" class="btn btn-sm btn-outline-info">Edit</a>
                    <a href="{{url('/delete_post/'.$post_show->id)}}" class="btn btn-sm btn-outline-danger" id="delete">Delete</a>
                    <a href="{{url('/show_post/'.$post_show->id)}}" class="btn btn-sm btn-outline-success">View</a>
                </td>
            </tr>
        @endforeach
        </tbody>
